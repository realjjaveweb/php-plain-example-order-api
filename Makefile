...
csfix-dry: ## checks php formatting in a "dry" run, only showing what files WOULD be changed; has silent=1 to hide fixer output
	@docker compose --file $(DOCKER_COMPOSE_FILE) exec api sh -c ' \
...
_csfix:
	...
	' # END OF sh -c
csfix: _csfix ## actually runs php formatting and does FILE CHANGES
	@$(MAKE) --no-print-directory csfix-dry silent=1
